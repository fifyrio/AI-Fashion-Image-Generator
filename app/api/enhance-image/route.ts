...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[enhance-image] Sending request to Replicate', replicateInput);
...
async function persistEnhancementResult(output: unknown): Promise<string | null> {
  ...
  if (directUrl) {
    try {
      ...
      console.warn('[enhance-image] Failed to download replicate result, fallback to direct URL:', error);
      return directUrl;
    }
  }
    ...
    if (output && 'url' in output) {
      ...
      if (typeof value === 'function') {
        try {
          ...
          console.warn('[enhance-image] Failed to read url() from output:', error);
        }
      }
